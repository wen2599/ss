// 文件名: email_worker.js
// 描述: Cloudflare Worker, 负责接收邮件，解析后安全地转发到 PHP 后端。

// --- START: Inlined postal-mime library ---
// Bundled version of postal-mime (https://github.com/js-choi/postal-mime)
// Included directly to avoid external module import issues in Cloudflare Workers.
const PostalMime = (() => { var t = function() { function t() { this.node = null, this.buffer = "", this.crlf = !1 } return t.prototype.write = function(t) { for (var r = 0; r < t.length; r++) { var e = t[r]; if (13 === e) this.crlf = !0; else { if (10 === e) { if (this.crlf) { var s = this.buffer.length > 0 ? new TextEncoder().encode(this.buffer) : new Uint8Array(0);
  ...
  export default {
    ...
    async email(message, env, ctx) {
      // 从环境变量中获取后端 Webhook URL 和密钥
      const BACKEND_EMAIL_WEBHOOK_URL = env.BACKEND_EMAIL_WEBHOOK_URL;
      const EMAIL_WEBHOOK_SECRET = env.EMAIL_WEBHOOK_SECRET;
  
      if (!BACKEND_EMAIL_WEBHOOK_URL || !EMAIL_WEBHOOK_SECRET) {
        console.error("Worker environment variables are not set!");
        message.setReject("Worker configuration error.");
        return;
      }
  
      try {
        // 使用内联的 PostalMime 库解析邮件流
        ...
        const payload = {
          ...
          // 优先使用纯文本正文，如果不存在则使用HTML正文
          raw_content: parsedEmail.text || parsedEmail.html || '',
        };
  
        // 发送 POST 请求到 PHP 后端
        const response = await fetch(BACKEND_EMAIL_WEBHOOK_URL, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Webhook-Secret': EMAIL_WEBHOOK_SECRET, // 发送安全密钥
          },
        ...
        if (!response.ok) {
          ...
          console.error(`Backend returned an error: ${response.status}. Body: ${errorText}`);
          // 告知邮件服务器处理失败，邮件可能会被退回或重试
        ...
        message.setReject(`Worker internal error.`);
      }
    },
  };