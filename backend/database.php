...
class Database {
    ...
    public static function findUserByToken($token) {
        $sql = "SELECT * FROM users WHERE auth_token = ? AND token_expires_at > NOW()";
        try {
            $stmt = self::getConnection()->prepare($sql);
            $stmt->execute([$token]);
            return $stmt->fetch();
    ...
    public static function getEmailsByUserId($user_id) {
        $sql = "SELECT from_email, to_email, subject, body, received_at FROM emails WHERE user_id = ? ORDER BY received_at DESC";
        try {
            $stmt = self::getConnection()->prepare($sql);
            $stmt->execute([$user_id]);
            return $stmt->fetchAll();
        } catch (\PDOException $e) {
            error_log("Error fetching emails by user ID: " . $e->getMessage());
