...
set_exception_handler(function ($exception) {
    error_log("Webhook Exception: " . $exception->getMessage() . " in " . $exception->getFile() . " on line " . $exception->getLine());
...
function getDbConnection(): ?mysqli {
    // Suppress errors with @ and check connect_error manually
...
if (!TELEGRAM_WEBHOOK_SECRET || TELEGRAM_WEBHOOK_SECRET !== $secretHeader) {
    ...
    error_log('Unauthorized webhook attempt. Secret was not provided or did not match.');
    exit;
}

// 2. Route request based on content
