...
switch ($endpoint) {
    case 'transfer_points':
        ...
        if (!isset($_SESSION['user_id'])) Response::send_json_error(401, '请先登录');
        $sender_id = $_SESSION['user_id'];
        $recipient_display_id = $data['recipientId'] ?? null;
        $amount = (int)($data['amount'] ?? 0);
        if (!$recipient_display_id || $amount <= 0) Response::send_json_error(400, '无效的接收人ID或金额');
        ...
        try {
            ...
            Response::send_json(['success' => true, 'message' => "成功赠送 {$amount} 积分"]);
        } catch (Exception $e) {
            $db->rollback();
            Response::send_json_error(400, $e->getMessage());
        }
        break;
    ...
    case 'find_user':
        ...
        if (!$phone) Response::send_json_error(400, '需要提供手机号');
        ...
        if ($user) {
            Response::send_json(['success' => true, 'user' => ['displayId' => $user['display_id']]]);
        } else {
            Response::send_json_error(404, '未找到该用户');
        }
        break;
}
