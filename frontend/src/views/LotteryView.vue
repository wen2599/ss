<template>
  <div>
    ...
    <!-- Error State -->
    <div v-else-if="error" class="bg-white rounded-lg shadow-md overflow-hidden text-center p-8 bg-red-50 border-red-200">
      <h3 class="text-lg font-semibold text-red-700">加载失败</h3>
      <p class="text-red-600 mt-2">{{ error }}</p>
    </div>

    <!-- No Results State -->
    <div v-else-if="results.length === 0" class="bg-white rounded-lg shadow-md overflow-hidden text-center p-8">
    ...
    <div v-else class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
      <div v-for="result in results" :key="result.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
        <!-- Card Header -->
